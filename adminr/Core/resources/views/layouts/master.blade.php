...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('adminr/css/adminr-core.css') }}">
    @stack('scopedCss')
</head>
<body class="c-app">
...
<script src="{{ asset('adminr/js/adminr-core.js') }}"></script>
@if(session('success'))
    <script>toast.success("{{ session('success') }}")</script>
@endif
@if(session('error'))
    <script>toast.error("{{ session('error') }}")</script>
@endif
<script>
