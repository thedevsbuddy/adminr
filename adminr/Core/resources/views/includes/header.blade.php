		...
		@can('manage_resources')
			@if(Adminr::inDev())
				<li class="c-header-nav-item">
					<a class="c-header-nav-link" href="{{ route('adminr.builder') }}" data-toggle="tooltip" title="{{ __('Resource Builder') }}">
						<svg style="width: 20px; height: 20px">
							<use xlink:href="{{ adminrIcon('cil-diamond') }}"></use>
						</svg>
						<span class="ml-2">{{ __('Builder') }}</span>
					</a>
				</li>
				<li class="c-header-nav-item">
					<a class="c-header-nav-link" href="{{ route('adminr.resources.index') }}" data-toggle="tooltip" title="{{ __('Generated Resources') }}">
						<svg style="width: 20px; height: 20px">
							<use xlink:href="{{ adminrIcon('cil-apps') }}"></use>
						</svg>
				...
				<li class="c-header-nav-item dropdown">
					...
					<div class="dropdown-menu dropdown-menu-right pt-0">
						...
						@can('manage_mail_templates')
							<x-link class="dropdown-item" href="{{ route('adminr.templates.index') }}">
								{{ __('Email Templates') }}
							</x-link>
						@endcan
						<x-link class="dropdown-item" href="{{ route('adminr.settings.general') }}">
							{{ __('General') }}
						</x-link>
						<x-link class="dropdown-item" href="{{ route('adminr.settings.email') }}">
							{{ __('Email') }}
						</x-link>
						<x-link class="dropdown-item" href="{{ route('adminr.settings.features') }}">
							{{ __('Features') }}
						</x-link>
					</div>
				</li>
			...
			<li class="c-header-nav-item dropdown">
				...
				<div class="dropdown-menu dropdown-menu-right pt-0">
					...
					<a class="dropdown-item" href="#">
						<x-aicon name="bell" class="c-icon mr-2"/>
					...
					<x-link class="dropdown-item" href="{{ route('auth.logout') }}" as="form" method="POST">
						<x-aicon name="account-logout" class="c-icon mr-2"/>
						{{ __('Logout') }}
					</x-link>
				</div>
			</li>
