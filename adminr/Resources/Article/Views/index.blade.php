            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <a href="{{ route( 'adminr.articles.create') }}" class="btn btn-sm btn-primary m-0">
                             <svg class="h-3 w-3">
                ...
                <div class="card-body p-0">
                    <table class="table table-bordered table-responsive-sm table-striped m-0">
                        ...
                        <tbody>
                                        ...
                                        @if(request()->has('trashed') && request()->get('trashed') == "true")
                                            <a href="javascript:void(0)" onclick="event.preventDefault();document.getElementById('article_restore_{{ $article->id }}').submit();" class="btn btn-sm mr-2 btn-icon btn-success" title="Restore">
                                                <svg class="h-3 w-3" style="transform: rotateY(180deg)">
                                                    <use xlink:href="{{ adminrIcon('cil-reload') }}"></use>
                                                </svg>
                                            </a>
                                            <form class="d-none" id="article_restore_{{ $article->id }}" action="{{ route( 'adminr.articles.restore', $article->id) }}" method="POST">
                                                @csrf
                                            </form>
                                            <a href="#" data-form="article_forceDelete_{{ $article->id }}" class="btn btn-sm mr-2 btn-icon btn-danger delete-item" title="Delete permanently">
                                                <svg class="h-3 w-3">
                                                    <use xlink:href="{{ adminrIcon('cil-trash') }}"></use>
                                                </svg>
                                            </a>
                                            <form class="d-none" id="article_forceDelete_{{ $article->id }}" action="{{ route( 'adminr.articles.force-destroy', $article->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        @else
                                            <a href="{{ route( 'adminr.articles.edit', $article->id) }}" class="btn btn-sm mr-2 btn-icon btn-primary" title="Edit">
                                                <svg class="h-3 w-3">
                                                    <use xlink:href="{{ adminrIcon('cil-pen') }}"></use>
                                                </svg>
                                            </a>
                                            ...
                                            <form class="d-none" id="article_{{ $article->id }}" action="{{ route( 'adminr.articles.destroy', $article->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        @endif
                            ...
                            @empty
                                <tr><td colspan="8" class="text-center">No data available for Articles</td></tr>
                            @endforelse
                        </tbody>
                        <tfoot>
